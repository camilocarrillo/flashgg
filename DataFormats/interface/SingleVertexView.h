...
public:
    ...
    const cand_type& pos() const {
        vertexMaybe();
        return *vtx_;
    }

    const cand_type* operator->() const {
        vertexMaybe();
        return vtx_;
    }
    ...
    float logSumPt2() const {
        vertexMaybe();
        return dipho_->logSumPt2(ivtx_);
    }
    float ptBal() const {
        vertexMaybe();
        return dipho_->ptBal(ivtx_);
    }
    float ptAsym() const {
        vertexMaybe();
        return dipho_->ptAsym(ivtx_);
    }
    float nConv() const {
        vertexMaybe();
        return dipho_->nConv(ivtx_);
    }
    float pullConv() const {
        vertexMaybe();
        return dipho_->pullConv(ivtx_);
    }
    float mva() const {
        vertexMaybe();
        return dipho_->mva(ivtx_);
    }

    bool isClosestToGen() const {
        return isClosestToGen_;
    }
    float dZtoGen() const {
        return dZtoGen_;
    }

    void setDzToGen(float x) {
        dZtoGen_=x;
    };
    void setIsClosestToGen(bool x=true) {
        isClosestToGen_=x;
    };

    ...
    if (TagSorter.product()->size() > 0 ) //make sure TagSorter is not a null pointer
    {
        ...
        if (itype !=0 ) {
            ...
            for( unsigned int genLoop =0 ; genLoop < gens.size(); genLoop++) {
                ...
                if ((fabs(gens[genLoop]->pdgId()) >8) && (count ==1)) {
                    count =0;
                    break;
                }
                if(count) {
...
//std::cout << "[DEBUG] " << vbfQuarkIndices.size() << std::endl;
            ...
            for(unsigned int ip=0; ip<gens.size(); ++ip) {
                ...
                if( diPhotonPointers[candIndex]->leadingPhoton()->et()< 20. || fabs(diPhotonPointers[candIndex]->leadingPhoton()->eta()) > 3. ) {
                    continue;
                }
            ...
            for(unsigned int ip=0; ip<gens.size(); ++ip) {
                ...
                if( diPhotonPointers[candIndex]->subLeadingPhoton()->et()< 20. || fabs(diPhotonPointers[candIndex]->subLeadingPhoton()->eta()) > 3. ) {
                    continue;
                }
                if ( gens[ip]->mother(0) != NULL && gens[ip]->mother(0)->pdgId() <= 25) {

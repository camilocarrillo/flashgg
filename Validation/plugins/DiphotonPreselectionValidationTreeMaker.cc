...
DiphotonPreselectionValidationTreeMaker::endJob()
{
    ...
    for( int ilead = 0 ; ilead < 2 ; ilead++ )
    {
        ...
        edm::LogVerbatim ("EfficienciesInfo") << "allcat" << "\t" << ntot_processed << "\t" << ntot_preselected << "\t" << ntot_passing_hoe << "\t" << ntot_passing_sieie << "\t" << ntot_passing_hcalISO << "\t" << ntot_passing_trkISO << "\t" << ntot_passing_pfISO;
        for( int icat = 0 + ilead * 4; icat < 4 + ilead * 4; icat++ )
        {
            edm::LogVerbatim ("EfficienciesInfo") << "cat" << icat % 4 << "\t" << nphoton_processed[icat] << "\t" << nphoton_preselected[icat] << "\t" << nphoton_passing_hoe[icat] << "\t" << nphoton_passing_sieie[icat] << "\t" << nphoton_passing_hcalISO[icat] << "\t" << nphoton_passing_trkISO[icat] << "\t" << nphoton_passing_pfISO[icat];
        ...
        for( int icat = 0 + ilead * 4; icat < 4 + ilead * 4; icat++ )
        {
            ...
            edm::LogVerbatim ("EfficienciesInfo") << "cat" << icat % 4 << "\t" << nphoton_processed[icat] << "\t" << (float)nphoton_preselected[icat]/(float)nphoton_processed[icat] << "\t" << (float)nphoton_passing_hoe[icat]/(float)nphoton_processed[icat] << "\t" << (float)nphoton_passing_sieie[icat]/(float)nphoton_processed[icat] << "\t" << (float)nphoton_passing_hcalISO[icat]/(float)nphoton_processed[icat] << "\t" << (float)nphoton_passing_trkISO[icat]/(float)nphoton_processed[icat] << "\t" << (float)nphoton_passing_pfISO[icat]/(float)nphoton_processed[icat];
        }

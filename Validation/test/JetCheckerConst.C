...
void JetCheckerConst() {
    ...
    JetTree_PU20bx25["PF"]      = (TTree*)f_PU20bx25->Get("flashggPFCollAnalyzer/tree_PF");
    JetTree_PU20bx25["PFCHS0"]  = (TTree*)f_PU20bx25->Get("flashggPFCollAnalyzer/tree_PFCHS0");
    JetTree_PU20bx25["PFCHSLeg"]= (TTree*)f_PU20bx25->Get("flashggPFCollAnalyzer/tree_PFCHSLeg");
    JetTree_PU20bx25["PUPPI0"]  = (TTree*)f_PU20bx25->Get("flashggPFCollAnalyzer/tree_PUPPI0");
    JetTree_PU20bx25["PUPPILeg"]= (TTree*)f_PU20bx25->Get("flashggPFCollAnalyzer/tree_PUPPILeg");
    ...
    JetTree_PU40bx50["PF"]      = (TTree*)f_PU40bx50->Get("flashggPFCollAnalyzer/tree_PF");
    JetTree_PU40bx50["PFCHS0"]  = (TTree*)f_PU40bx50->Get("flashggPFCollAnalyzer/tree_PFCHS0");
    JetTree_PU40bx50["PFCHSLeg"]= (TTree*)f_PU40bx50->Get("flashggPFCollAnalyzer/tree_PFCHSLeg");
    JetTree_PU40bx50["PUPPI0"]  = (TTree*)f_PU40bx50->Get("flashggPFCollAnalyzer/tree_PUPPI0");
    JetTree_PU40bx50["PUPPILeg"]= (TTree*)f_PU40bx50->Get("flashggPFCollAnalyzer/tree_PUPPILeg");

    // ===> Only for def
    for (std::map<TString,TTree*>::iterator it=JetTree_PU20bx25.begin(); it!=JetTree_PU20bx25.end(); ++it) {
        h_PU20bx25_eta   [it->first] = new TH1F(Form("h_PU20bx25_eta_%s" ,it->first.Data()),Form("%s;eta",it->first.Data()),150,-6,6);
        ...
        h_PU40bx50_eta   [it->first] = new TH1F(Form("h_PU40bx50_eta_%s" ,it->first.Data()),Form("%s;eta",it->first.Data()),150,-6,6);
        ...
        h_PU40bx50_eta_corr[it->first] = new TH2F(Form("h_PU40bx50_eta_corr_%s",it->first.Data()),Form("%s;#eta;#Delta p_{t}",it->first.Data()),300,-6,6,200,-40,40);
        h_PU20bx25_eta_corr[it->first] = new TH2F(Form("h_PU20bx25_eta_corr_%s",it->first.Data()),Form("%s;#eta;#Delta p_{t}",it->first.Data()),300,-6,6,200,-40,40);
    ...
    for (std::map<TString,TTree*>::iterator it=JetTree_PU20bx25.begin(); it!=JetTree_PU20bx25.end(); ++it) {
        ...
        h_PU20bx25_eta[it->first]->GetYaxis()->SetRangeUser(0,50000);
        h_PU20bx25_eta[it->first]->Draw();
        h_PU40bx50_eta[it->first]->Draw("same");

        Canv["eta_" + it->first]->SaveAs(Form("plots/const_eta_%s.C",it->first.Data()));
        Canv["eta_" + it->first]->SaveAs(Form("plots/cosnt_eta_%s.pdf",it->first.Data()));
        ...
        Canv["pt_"+it->first]->SaveAs(Form("plots/const_pt_%s.pdf",it->first.Data()));
        Canv["pt_"+it->first]->SaveAs(Form("plots/const_pt_%s.C",it->first.Data()));
        ...
        Canv["mass_"+it->first]->SaveAs(Form("plots/const_mass_%s.pdf",it->first.Data()));
        Canv["mass_"+it->first]->SaveAs(Form("plots/const_mass_%s.C",it->first.Data()));

...
void JetPerformanceStudies_ETA(int catbin=1, bool withPUJID=true) {
    ...
    TCut cut_PUJID          = "passesPUJetID";
    TCut cut_pt             = "bestPt>20.";
    TCut cut_gen_pt         = "pt>20.";
    TCut cut_genjetmatch    = "genJetMatch";
    TCut cut_recojetmatch   = "recoJetMatch";
    ...
    for (std::map<TString,TTree*>::iterator it=JetTree.begin(); it!=JetTree.end(); ++it) {
        ...
        if(withPUJID) {
            GenJetTree[it->first]->Draw(Form("eta>>h_effeta_sel_%s",it->first.Data()), cut_recojetmatch && cut_gen_all && cut_PUJID,"");
        } else {
